...
impl<K, V> ArchivedHashMap<K, V> {
    ...
    /// There can be situations where you are unable to construct a key that fulfils the criteria of `get`, such as when using
    /// a tuple of `ArchivedString` as the key.
    /// Instead you can use this method. It takes in
    /// If you are unable to construct a key that fulfils the criteria of `get`, you can use this
    /// method instead.
    ///
    /// Hashes are not always guaranteed to match between Archived and non-archived versions of the same struct.
    /// This is also the case for Archived types that are automatically generated by `#[derive(Archive)]`
    /// You should ensure that the hashing method matches between your types, otherwise this method might not return any value
    /// even though a value matching `comparison_predicate` exists.
    ///
    /// # Examples
    ///
    /// ```no-run
