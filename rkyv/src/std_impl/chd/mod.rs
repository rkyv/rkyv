...
/// A structure that can convert a `Vec` to `ArchivedHashMap`
///
/// rkyv's `ArchivedHashMap` uses a fairly different implementation than `HashMap` in the standard
/// library. Therefore, constructing `HashMap` and converting it to `ArchivedHashMap` is not very
/// cheap. If a set of values needs to be archived as `ArchivedHashMap`, then converting it to
/// `VecForHashMap` and serialize it might be the best choice. When archiving it, you can simply
/// specify `HashMap<K, V>` as generic parameter to make it an `ArchivedHashMap`. For example,
///
/// ```rust
/// # use rkyv::{AlignedVec, std_impl::chd::VecForHashMap, ser::serializers::AlignedSerializer, archived_root, ser::Serializer};
/// # use std::collections::HashMap;
/// let mut serializer = AlignedSerializer::new(AlignedVec::new());
/// let vector = (0..5).into_iter().map(|x| (x, 0u8)).collect::<Vec<_>>();
/// let hashmap = VecForHashMap::new(vector);
/// serializer.serialize_value(&hashmap).unwrap();
/// let buffer = serializer.into_inner();
/// let archived_value = unsafe {
///     archived_root::<HashMap<usize, u8>>(&buffer)
/// };
/// for i in 0..5 {
///     assert_eq!(archived_value.get(&i), Some(&0u8))
/// }
/// ```
pub struct VecForHashMap<'a, K, V, KU: 'a + Hash + Eq, VU: 'a> {
    vector: Vec<(KU, VU)>,
    _phantom: PhantomData<(&'a K, &'a V, &'a KU, &'a VU)>
}
impl<'a, K, V, KU: 'a + Hash + Eq, VU: 'a> VecForHashMap<'a, K, V, KU, VU> {
    pub fn new(vector: Vec<(KU, VU)>) -> Self {
        Self {
            vector,
...
impl<'a, K, V, KU: 'a + Archive<Archived = K> + Hash + Eq, VU: 'a + Archive<Archived = V>> Archive for VecForHashMap<'a, K, V, KU, VU>
{
    type Archived = ArchivedHashMap<KU::Archived, VU::Archived>;
    type Resolver = ArchivedHashMapResolver;

    #[inline]
    fn resolve(&self, pos: usize, resolver: Self::Resolver, out: &mut MaybeUninit<Self::Archived>) {
        resolver.resolve_from_len(pos, self.vector.len(), out);
    }
}

impl<'a, K, V, KU: 'a + Serialize<S, Archived = K> + Hash + Eq, VU: 'a + Serialize<S, Archived = V>, S: Serializer + ?Sized> Serialize<S>
    for VecForHashMap<'a, K, V, KU, VU> where K: Hash + Eq
{
    #[inline]
    fn serialize(&self, serializer: &mut S) -> Result<Self::Resolver, S::Error> {
        ArchivedHashMap::serialize_from_iter(self.vector.iter().map(|(x, y)| (x, y)), self.vector.len(), serializer)
    }
}
