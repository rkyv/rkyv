...
use crate::{
    ...
    Archive, Archived, Deserialize, Fallible, Serialize, vec::{VecResolver, ArchivedVec}, primitive::ArchivedUsize,
};
...
//         let deserialized: BitVec = output.deserialize(Strategy::<_, Infallible>::wrap(&mut ())).unwrap();
...
//             output.deserialize(Strategy::<_, Infallible>::wrap(&mut ())).unwrap();
