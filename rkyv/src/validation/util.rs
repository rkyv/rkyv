...
use rancor::{Strategy, Error};

use crate::{Archive, validation::{ArchiveContext, validators::DefaultValidator, ArchiveContextExt as _}, de::deserializers::SharedDeserializeMap, Deserialize};
...
///
/// [unsafe_version]: crate::access_pos_unchecked
#[inline]
...
{
    let mut validator = DefaultValidator::new(bytes);
    access_with_context::<T, DefaultValidator, E>(
        bytes,
        &mut validator,
    )
}
...
/// situation.
///
/// This is a safe alternative to [`deserialize_unchecked`][unsafe_version].
///
/// [unsafe_version]: crate::deserialize_unchecked
///
/// # Examples
/// ```
...
/// ```
#[inline]
pub fn deserialize<T, E>(bytes: &[u8]) -> Result<T, E>
where
    T: Archive,
    T::Archived: CheckBytes<Strategy<DefaultValidator, E>> + Deserialize<T, Strategy<SharedDeserializeMap, E>>,
    E: Error,
{
    let mut deserializer = SharedDeserializeMap::default();
    access::<T, E>(bytes)?.deserialize(Strategy::wrap(&mut deserializer))
}
