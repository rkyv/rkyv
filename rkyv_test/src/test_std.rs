...
mod tests {
    ...
    use rkyv::{
        ...
        util::{AlignedBytes, serialize_using}, Archive, Deserialize, Serialize, rancor::Failure, serialize,
    };
    ...
    fn write_serializer() {
        ...
        serialize_using::<_, _, Failure>(&foo, &mut ser)
    ...
    fn archive_hash_map() {
        ...
        let buf = serialize::<_, 256, Failure>(&hash_map).unwrap();
        let archived_value =
            unsafe { access_unchecked::<HashMap<String, String>>(buf.as_ref()) };
    ...
    fn archive_hash_set() {
        ...
        let buf = serialize::<_, 256, Failure>(&hash_set).unwrap();
        let archived_value =
    ...
    fn archive_hash_set_hasher() {
        ...
        let buf = serialize::<_, 256, Failure>(&hash_set).unwrap();
        let archived_value = unsafe {
            access_unchecked::<HashSet<String, ahash::RandomState>>(buf.as_ref())
        };
