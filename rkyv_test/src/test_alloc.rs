...
mod tests {
    ...
    use core::{pin::Pin, convert::Infallible};
    use rkyv::{
        ...
        util::{AlignedBytes, AlignedVec}, Archive, Archived, Deserialize, Serialize, rancor::{Error, Fallible, Strategy, Failure}, serialize,
    };
    ...
    mod isolate {
        use rkyv::{rancor::Failure, util::serialize_into};
        #[cfg(feature = "wasm")]
        use wasm_bindgen_test::*;
        ...
        fn archive_example() {
            ...
            let bytes = rkyv::serialize::<_, 256, Failure>(&value).unwrap();

            // Or you can customize your serialization for better performance
            // and compatibility with #![no_std] environments
            use rkyv::ser::serializers::AllocSerializer;

            let mut serializer = serialize_into::<_, _, Failure>(
                &value,
            ...
            let archived =
                rkyv::access::<Test, Failure>(&bytes[..]).unwrap();
            ...
            let deserialized: Test = archived.deserialize(rkyv::rancor::Strategy::<_, Failure>::wrap(&mut ())).unwrap();
    ...
    fn archive_derives() {
        ...
        let buf = serialize::<_, 0, Failure>(&value)
    ...
    fn basic_mutable_refs() {
        let mut buf = serialize::<_, 0, Failure>(&42i32).unwrap();
        let mut value = unsafe {
            access_unchecked_mut::<i32>(Pin::new(buf.as_mut()))
        };
        assert_eq!(*value, 42);
        *value = 11.into();
        assert_eq!(*value, 11);
    ...
    fn struct_mutable_refs() {
        ...
        let mut buf = serialize::<_, 256, Failure>(&value).unwrap();
        let mut value =
    ...
    fn enum_mutable_ref() {
        ...
        let mut buf = serialize::<_, 0, Failure>(&value).unwrap();
        let mut value =
            unsafe { access_unchecked_mut::<Test>(Pin::new(buf.as_mut())) };
    ...
    fn check_util_bounds() {
        fn check<T: Fallible + Serializer>() {}
    ...
    fn arbitrary_enum_discriminant() {
        ...
        let archived = unsafe { access_unchecked::<ReallyBigEnum>(buf.as_ref()) };
        assert_eq!(archived, &ArchivedReallyBigEnum::V100);

        let deserialized: ReallyBigEnum =
            archived.deserialize(&mut ()).unwrap();
    ...
    mod with {
        ...
        use core::{str::FromStr, convert::Infallible};
        use rkyv::{
            access_unchecked,
            ser::serializers::AlignedSerializer,
            ser::Serializer,
            with::{ArchiveWith, DeserializeWith, SerializeWith},
            AlignedVec, Archive, Archived, Deserialize,
            Serialize, rancor::{Fallible, Strategy},
        };
    ...
    fn with_copy_optimize() {
        ...
        let deserialized: Test = archived.deserialize(Strategy::<_, Infallible>::wrap(&mut ())).unwrap();
